                                ...
                                <td class="px-4 py-3 text-end">
                                    <div x-data="{ o: false }" @click.outside="o = false" class="relative inline-block">
                                        ...
                                        <div x-show="o" x-cloak class="absolute end-0 z-10 mt-1 w-36 overflow-hidden rounded-lg border border-line bg-surface py-1 shadow-lg">
                                            <button wire:click="openEdit('{{ $t['uuid'] }}')" @click="o=false" class="flex w-full items-center gap-2 px-3 py-2 text-start text-sm text-fg hover:bg-surface-2"><x-icon name="pencil" class="size-4" />{{ __('emp.timeTracking.adjust') }}</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
    ...
    <x-ui.slide-over wire="showForm" :title="$editingUuid ? __('emp.timeTracking.editTitle') : __('emp.timeTracking.createTitle')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            ...
            <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                ...
                <x-ui.button type="submit" wire:loading.attr="disabled" wire:target="save">{{ $editingUuid ? __('emp.timeTracking.saveChanges') : __('emp.timeTracking.create') }}</x-ui.button>
            </div>
        </form>
    </x-ui.slide-over>
</div>

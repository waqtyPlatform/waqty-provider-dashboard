                            ...
                            @foreach ($this->visibleRules as $rule)
                                <tr wire:key="rule-{{ $rule['uuid'] }}" class="border-b border-line last:border-0 hover:bg-surface-2">
                                    ...
                                    <td class="px-4 py-3 text-end">
                                        <x-ui.dropdown>
                                            <x-ui.dropdown-item icon="pencil" wire:click="openEdit('{{ $rule['uuid'] }}')">{{ __('common.edit') }}</x-ui.dropdown-item>
                                            <x-ui.dropdown-item icon="trash-2" wire:click="confirmDelete('{{ $rule['uuid'] }}')" destructive>{{ __('common.delete') }}</x-ui.dropdown-item>
                                        </x-ui.dropdown>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <x-ui.slide-over wire="showForm" :title="$formTitles[$form_kind] ?? ''">
            <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
                ...
                <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                    ...
                    <x-ui.button type="submit" loadingTarget="save">{{ $editingUuid ? __('emp.commissions.saveChanges') : __('emp.commissions.create') }}</x-ui.button>
                </div>
            </form>
        </x-ui.slide-over>

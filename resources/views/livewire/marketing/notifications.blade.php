                        ...
                        @foreach ($this->items as $n)
                            <tr wire:key="ntf-{{ $n['id'] }}" class="border-b border-line last:border-0 hover:bg-surface-2">
                                ...
                                <td class="px-4 py-3 text-end">
                                    <x-ui.dropdown>
                                        <x-ui.dropdown-item icon="pencil" wire:click="openEdit('{{ $n['id'] }}')">{{ __('common.edit') }}</x-ui.dropdown-item>
                                        <x-ui.dropdown-item icon="trash-2" wire:click="confirmDelete('{{ $n['id'] }}')" destructive>{{ __('common.delete') }}</x-ui.dropdown-item>
                                    </x-ui.dropdown>
                                </td>
                            </tr>
                        @endforeach
    ...
    <x-ui.slide-over wire="showForm" :title="$editingId ? __('mkt.notifications.edit') : __('mkt.notifications.create')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            <div class="flex-1 space-y-4 p-5">
                <x-ui.input :label="__('mkt.notifications.lblTitle')" wire:model="form_title" maxlength="80" :placeholder="__('mkt.notifications.titlePh')" required :error="$errors->first('form_title')" />
                <div>
                ...
                <x-ui.select :label="__('mkt.notifications.lblAudience')" wire:model="form_audience" required :options="$audiences" :error="$errors->first('form_audience')" />
            </div>

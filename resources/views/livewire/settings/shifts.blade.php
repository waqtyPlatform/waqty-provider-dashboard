                            ...
                            <td class="px-4 py-3 text-end">
                                <div x-data="{ o: false }" @click.outside="o = false" class="relative inline-block">
                                    ...
                                    <div x-show="o" x-cloak class="absolute end-0 z-10 mt-1 w-36 overflow-hidden rounded-lg border border-line bg-surface py-1 shadow-lg">
                                        <button wire:click="openEdit('{{ $t->uuid }}')" @click="o=false" class="flex w-full items-center gap-2 px-3 py-2 text-start text-sm text-fg hover:bg-surface-2"><x-icon name="pencil" class="size-4" />{{ __('common.edit') }}</button>
                                        <button wire:click="confirmDelete('{{ $t->uuid }}')" @click="o=false" class="flex w-full items-center gap-2 px-3 py-2 text-start text-sm text-error hover:bg-error-light"><x-icon name="trash-2" class="size-4" />{{ __('common.delete') }}</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    <x-ui.slide-over wire="showForm" :title="$editingUuid ? __('settings.shiftTemplates.editTitle') : __('settings.shiftTemplates.createTitle')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            <div class="flex-1 space-y-4 p-5">
                <x-ui.input :label="__('settings.shiftTemplates.templateName')" wire:model="form_name" :placeholder="__('settings.shiftTemplates.namePh')" :error="$errors->first('form_name')" />
            ...
            <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                ...
                <x-ui.button type="submit">{{ $editingUuid ? __('settings.shiftTemplates.saveChanges') : __('settings.shiftTemplates.createBtn') }}</x-ui.button>
            </div>
        </form>
    </x-ui.slide-over>

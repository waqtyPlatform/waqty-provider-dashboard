                    ...
                    @foreach ($this->items as $s)
                        <tr wire:key="safe-{{ $s->uuid }}" class="border-b border-line last:border-0 hover:bg-surface-2">
                            ...
                            <td class="px-4 py-3 text-end">
                                <x-ui.dropdown>
                                    <x-ui.dropdown-item icon="pencil" wire:click="openEdit('{{ $s->uuid }}')">{{ __('common.edit') }}</x-ui.dropdown-item>
                                    <x-ui.dropdown-item icon="trash-2" wire:click="confirmDelete('{{ $s->uuid }}')" destructive>{{ __('common.delete') }}</x-ui.dropdown-item>
                                </x-ui.dropdown>
                            </td>
                        </tr>
                    @endforeach
    ...
    <x-ui.slide-over wire="showForm" :title="$editingUuid ? __('settings.safes.editTitle') : __('settings.safes.createTitle')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            <div class="flex-1 space-y-4 p-5">
                <x-ui.input :label="__('settings.safes.safeName')" wire:model="form_name" :placeholder="__('settings.safes.namePh')" :error="$errors->first('form_name')" :required="true" />
                <x-ui.input :label="__('settings.safes.branch')" wire:model="form_branch" :error="$errors->first('form_branch')" />
                <x-ui.input type="number" :label="$editingUuid ? __('settings.safes.currentBalance') : __('settings.safes.initialBalance')" wire:model="form_balance" min="0" step="0.01" :error="$errors->first('form_balance')" :required="true" />
            ...
            <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                ...
                <x-ui.button type="submit" loadingTarget="save">{{ $editingUuid ? __('settings.safes.saveChanges') : __('settings.safes.createSafe') }}</x-ui.button>
            </div>
        </form>
    </x-ui.slide-over>

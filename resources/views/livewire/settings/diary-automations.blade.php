    ...
    <x-ui.slide-over wire="showForm" :title="$editingUuid ? __('settings.diaryAutomations.editTitle') : __('settings.diaryAutomations.createTitle')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            <div class="flex-1 space-y-4 p-5">
                <x-ui.input :label="__('settings.diaryAutomations.autoName')" wire:model="form_name" :placeholder="__('settings.diaryAutomations.namePh')" :error="$errors->first('form_name')" :required="true" />
                <x-ui.select :label="__('settings.diaryAutomations.triggerLabel')" wire:model="form_trigger" :options="$triggerLabels" :error="$errors->first('form_trigger')" :required="true" />
                <x-ui.select :label="__('settings.diaryAutomations.actionLabel')" wire:model="form_action" :options="$actionLabels" :error="$errors->first('form_action')" :required="true" />
            ...
            <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                ...
                <x-ui.button type="submit" loadingTarget="save">{{ $editingUuid ? __('settings.diaryAutomations.saveChanges') : __('settings.diaryAutomations.saveAutomation') }}</x-ui.button>
            </div>
        </form>
    </x-ui.slide-over>

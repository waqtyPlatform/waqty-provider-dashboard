    ...
    <x-ui.slide-over wire="showForm" :title="$editingUuid ? __('settings.fpAreas.editTitle') : __('settings.fpAreas.createTitle')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            <div class="flex-1 space-y-4 p-5">
                <x-ui.input :label="__('settings.fpAreas.areaName')" wire:model="form_name" :placeholder="__('settings.fpAreas.namePh')" :error="$errors->first('form_name')" :required="true" />
            ...
            <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                ...
                <x-ui.button type="submit" loadingTarget="save">{{ $editingUuid ? __('settings.fpAreas.saveChanges') : __('settings.fpAreas.createArea') }}</x-ui.button>
            </div>
        </form>
    </x-ui.slide-over>

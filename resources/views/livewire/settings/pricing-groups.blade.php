                    ...
                    @foreach ($this->items as $g)
                        <tr wire:key="pg-{{ $g->uuid }}" class="border-b border-line last:border-0 hover:bg-surface-2">
                            ...
                            <td class="px-4 py-3 text-end">
                                <x-ui.dropdown>
                                    <x-ui.dropdown-item icon="pencil" wire:click="openEdit('{{ $g->uuid }}')">{{ __('common.edit') }}</x-ui.dropdown-item>
                                    <x-ui.dropdown-item icon="trash-2" wire:click="confirmDelete('{{ $g->uuid }}')" destructive>{{ __('common.delete') }}</x-ui.dropdown-item>
                                </x-ui.dropdown>
                            </td>
                        </tr>
                    @endforeach
    ...
    <x-ui.slide-over wire="showForm" :title="$editingUuid ? __('settings.pricingGroups.editGroup') : __('settings.pricingGroups.addGroup')">
        <form wire:submit="save" class="flex flex-1 flex-col overflow-y-auto">
            <div class="flex-1 space-y-4 p-5">
                <x-ui.input :label="__('settings.pricingGroups.groupName')" wire:model="form_name" :placeholder="__('settings.pricingGroups.groupNamePlaceholder')" :error="$errors->first('form_name')" :required="true" />
            ...
            <div class="flex items-center justify-end gap-2 border-t border-line px-5 py-4">
                ...
                <x-ui.button type="submit" loadingTarget="save">{{ __('settings.pricingGroups.saveGroup') }}</x-ui.button>
            </div>
        </form>
    </x-ui.slide-over>
